...
**S1 · Login.** Receipt-glyph mark block (accent) — per deviation #6 the login mark is the **receipt glyph**, not the GE monogram the older render still shows (built code is correct), "Ganpati Enterprises / ORDER CAPTURE · FIELD SALES", mono field labels (USERNAME / PASSWORD + SHOW — **D9, 2026-07-07: username login, not email**; registration still uses email per D3), "Keep me signed in — ~30 DAYS ON THIS PHONE" checked by default, full-width accent Sign in. Footer: *"Forgot password? Call the office to reset it."* Errors (wrong username/password, deactivated account — same generic message) render as flat red-edged strips; submit shows an inline spinner in the accent block.
...
**S3 · Pick Retailer.** "Select retailer" — no subtitle, no step language (owner override, 2026-07-07: dropped "NEW ORDER · STEP 1 / 3" everywhere in the flow). Search "Shop name or area". `RECENT` group first, then `ALL SHOPS`; NEW tag (amber square family) on unverified shops. Bottom: outlined "+ Add new shop". Quick-add: SHOP NAME (only required field, red asterisk), AREA + PHONE side by side, accent-edged note *"Saved as NEW — pending verification. Order now; the office cleans up the record later."*, full-width "Add & start order"; typed no-results query carries into the name field. Resume-draft: flat bottom sheet, 2px ink top-edge (no rounded modal, no shadow): "Continue order for Sharma Electronics? · 3 items · ₹4,478 · saved 11:31 on this phone" → Continue order / Start fresh.

**S4 · Quick Order (hero).** Header: shop name + area only (e.g. "Sadar Bazaar"), back arrow — no "NEW ORDER", no step language (owner override, 2026-07-07); the header now constantly confirms *who* the order is for, cutting wrong-shop errors. Sticky search ("Search name or SKU", instant, client-side, name+SKU, case/space-insensitive; result meta "1 of 34 products"; group header stays for context). Category bars = accent-navy (`#1D4ED8`), full-bleed sticky mono labels with counts (`SPEAKERS · 10`) — the white band + hairline rule span edge-to-edge, pinned just under the search bar via a live-measured offset so the current category stays visible while scrolling (owner override, 2026-07-07 — was plain grey, non-sticky, inset). Row = name (2-line clamp) + mono price + stepper `[−] qty [+]`; **+ is filled accent once in cart**; in-cart row = saturated accent tint (owner override, 2026-07-07: strengthened from a pale, barely-visible tint) + 2px accent bar down the left edge + bold quantity. Tap qty → own keypad sheet (product name + current qty shown; Cancel / Set quantity; scrim-tap discards). Sticky cart bar splits: **black data half** (`3 items` over mono `₹4,478`) + **accent action half** (`Review ›`) — no floating pill. Cart bar survives filtering and never disappears mid-dictation. Item count = distinct lines. Row/category padding is deliberately tight (owner override, 2026-07-07 — denser ledger, more rows visible per screen) while the stepper's actual tap targets stay ≥48px regardless. States: default grouped, in-cart + longest-name clamp (…SOUNDBAR SPEAKER (ABABA 1) at 2 lines), searching "astra", empty search ("No products match 'xyz' / Check the spelling, or try part of the SKU" + Clear search — never a dead end), keypad open.

**S5 · Review.** "Review order" — no subtitle, no step language (owner override, 2026-07-07). Retailer confirmation header + "Change". Lines: name + mono amount; compact stepper + `@ ₹364` mono rate. `NOTES FOR THE OFFICE` with live `23/500` counter. `Total · 3 items` + mono total. Full-width accent "Submit order". Submit-failed/offline: CTA swaps to **amber Retry** over a flat amber-edged "Saved on phone — not submitted yet" strip; copy says retrying is idempotent ("Retrying never creates a duplicate").
...
5. **v1 vs instrument**: implement only the instrument grammar; use v1 sections (the `sec-s1…s8` renders — **`sec-s6` is absent**, so that range label is nominal, not a promise of eight files) solely as state checklists.
6. **Product mark — owner decision 2026-07-06:** the **receipt glyph** ([favicon.png](../assets/favicon.png) — zigzag-edged bill with two ink lines; the repo's official logo/favicon at [assets/favicon.png](../assets/favicon.png)) is the icon *everywhere*: favicon, add-to-home-screen, and the S1 login block. This **overrides** the designer's GE-monogram decision; render it in ink or accent per context. Android maskable icons need a padded safe-zone variant at build time.
