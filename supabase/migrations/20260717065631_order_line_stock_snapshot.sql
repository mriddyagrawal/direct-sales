-- Order-line stock snapshot (owner 2026-07-17): capture products.stock_qty at
-- order time as order_items.stock_at_order — a static historical fact (like
-- product_name/unit_price_paise), never recomputed. Advisory only: ordering
-- still never blocks or decrements stock.

-- 1. The snapshot column (nullable; instant, no rewrite).
alter table public.order_items add column stock_at_order integer;

-- 1b. Backfill every pre-feature line to its own qty → reads as in-stock (no
--     pill), never a misleading N/A. NEVER a flat constant. After this a NULL
--     can only mean "new order of a product with no Tally stock data".
update public.order_items set stock_at_order = qty where stock_at_order is null;

-- 2. submit_order — current body verbatim; the ONLY change is stock_at_order
--    (= v_product.stock_qty) added to the order_items insert.
...
begin
  ...
  for v_item in select * from jsonb_array_elements(p_items) loop
    ...
    insert into public.order_items (order_id, product_id, product_name, unit_price_paise, qty, line_total_paise, position, stock_at_order)
    values (p_id, v_product.id, v_product.name, v_unit_price, v_qty, v_unit_price::bigint * v_qty, v_item_count, v_product.stock_qty);
...
--    stock_at_order (= v_product.stock_qty) on the NEW-LINE insert. The
--    existing-line UPDATE is untouched: an edit never rewrites an order-time
--    snapshot.
...
begin
  ...
  for v_item in select * from jsonb_array_elements(p_items) loop
    ...
    if exists (select 1 from public.order_items where order_id = p_order_id and product_id = v_product_id) then
      ...
      insert into public.order_items (order_id, product_id, product_name, unit_price_paise, qty, line_total_paise, position, stock_at_order)
      values (p_order_id, v_product.id, v_product.name, v_unit_price, v_qty, v_unit_price::bigint * v_qty, v_position, v_product.stock_qty);
    end if;
