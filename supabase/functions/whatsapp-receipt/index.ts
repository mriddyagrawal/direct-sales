...
//
//   1. The DB trigger (whatsapp_webhook() on deposit_events INSERT, action
//      'created', x-trigger-secret header): sends the approved
...
//      Sent ONLY when previous_outstanding_paise was entered (the template
//      requires it; that's the owner's rollout lever). Logged back into
//      deposit_events as receipt_sent / receipt_failed — visible in the
//      dashboard's edit-trail UI, and a safe no-op in `notify` (its
//      depositCards only knows created/voided).
//
