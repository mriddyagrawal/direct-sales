...
**Context.** The owner asked for a permissions overview across salesman/accountant/admin, framing admin's intended purpose as "oversee, and help in extreme cases" rather than day-to-day operation. Checking the actual RPCs (`submit_order`, `update_order_items`, `cancel_order`, `process_order`) showed every role check branches on `v_role in ('accountant', 'admin')` — no admin-only RPC path exists, and the dashboard UI/nav doesn't differentiate the two roles at all. **Correction (REVIEWER, same-day):** the RLS layer does have one real admin-only policy, `products_admin_insert` (inserting a **new** product row — accountant can only UPDATE existing ones); it's just dormant since no in-app screen calls it yet. The provisioning gap (creating users, setting `profiles.role`/`username`, done by hand in Supabase Studio) is the other admin-exclusive capability.

**Decision.** Leave it as is (owner-confirmed 2026-07-07). Admin and accountant keep effectively identical in-app permissions — the one latent exception (`products_admin_insert`) isn't reachable from any screen today. "Admin is for oversight/escalation, accountant runs the queue" remains a role the owner chooses to play, not something the system enforces — nothing technical stops an admin account from doing full-time accountant work, and no UI cue distinguishes the two.
