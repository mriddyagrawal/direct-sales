...
**Ambiguity rule (this is the money-safety rule):** if a payload key matches **more than one** retailer, update **neither**, and report the key in a third bucket, `ambiguous`. The stock functions don't do this — with a duplicate `tally_name` they'd update both rows, which for stock is merely wrong and for **money is a double-post into every roll-up**. Retailers get the stricter contract. The one known duplicate today will land in this bucket on day one, which is exactly how the owner learns to fix it.
