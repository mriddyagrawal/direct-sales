...
- **Admin (owner)** — intended as **oversight and escalation, not daily operation**: keep an eye on the queue, step in for exceptions, and handle the things only an owner should (creating/deactivating accounts, changing a profile's role, catalog administration beyond day-to-day pricing). **The M5.5 catalog-admin surfaces — "+ Add product" and Excel "Import" — are admin-only in-app** (both INSERT products via `products_admin_insert`, which is admin-scoped). The accountant sees the Products ledger and, in the M5.5 Add/Edit modal, edits price / `tally_name` / `active` on existing rows while `name` and `category` are read-only (`products_staff_update`), but has no Add or Import. **That name/category lock is UI-only (㉟):** `products_staff_update` is a row-level policy, so at the database layer the accountant *can* update `name`/`category` on an existing row via a crafted request — the read-only fields are a convention the modal presents, not a permission RLS enforces (Postgres RLS is row-level, not column-level). This is the same shape as D11 (admin/accountant separation is convention, not enforcement) and is left as-is by that decision; enforcing it for real would need a `BEFORE UPDATE` trigger rejecting `name`/`category` changes when `auth_profile_role() = 'accountant'` (nothing in the app relies on that today).
...
| `products` | SELECT where `active AND price_paise IS NOT NULL` — unpriced products are invisible at the database level (D2) | SELECT all; UPDATE existing rows — `products_staff_update` is **row-level (whole-row)**, so `name`/`category` are updatable too; the "price/`tally_name`/`active` only" limit is a **UI convention** (㉟), not a DB constraint | ALL |
