...
- **`sku` drop has a backend dependency (do it first).** The **only** function that emits `sku` in an `order_events` payload is **`update_order_items`** (2 sites — `before`/`after`); `submit_order`/`process_order`/`cancel_order` emit none (reviewer ㉞, verified live). Recreate **only** `update_order_items` from its **current 4-arg `p_reason` body** in `20260707T120000_update_order_items_reason.sql` — **not** the superseded 3-arg copies in `20260706T150400_rpcs.sql` / `20260706T150800_rename_current_role.sql` (copying those regresses the mandatory-`p_reason`-after-lock logic ㉘) — swapping its two `'sku'`→`'tally_name'`; update the event catalog in [specs/order-lifecycle.md](specs/order-lifecycle.md) (`{ tally_name, qty, unit_price_paise }`); **then** drop `sku`. Old events keep their `sku` key; new ones carry `tally_name`.
