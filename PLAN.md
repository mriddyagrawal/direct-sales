...
|---|---|---|---|
| ㉗(b) | HISTORY (S7 order detail) renders real staff names, not a generic "the office" — `profiles_select_active` (M1) lets any active staff be read, so this is intended behavior, not a bug | owner-confirm | Awaiting Mridul's call: real names (current) vs. always "the office" |
