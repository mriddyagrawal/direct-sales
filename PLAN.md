...
**Now — LIVE IN PRODUCTION + IN ACTIVE BUSINESS USE (2026-07-11).** The app is live at [direct-sales-nu.vercel.app](https://direct-sales-nu.vercel.app) (prod from `main`, single Supabase prod project `ugjwcbxyyuowiyhczcrh`; **both India-region now** — Supabase `ap-south-1` + Vercel `bom1`, Mumbai) and **being used by the business for real orders** (the `order_no` sequence has been running on real orders since rollout). Phase 1 (M0–M5), **M5.5 catalog admin**, **Phase 3a/3b multi-brand** (Zebronics + Luminous fixed, **LG** manual + admin approval), the **admin User Management** screen, and the **39/39-green role-phased E2E** (2026-07-09; 2 skipped by design, 0 defects — [tests/e2e-test-plan.md](tests/e2e-test-plan.md)) are all ✅ done and merged. **Real accounts exist** (3 salesmen, 1 accountant, 1 admin; username login, D9); **606 retailers** imported.

**Since E2E, a major lifecycle + fulfilment + mobile-UX stream shipped to `main`** (all reviewer-verified in [comments.md](comments.md)):
- **Lifecycle overhaul (2026-07-10):** one universal **admin-only approval** for *every* brand; dropped the old `submitted` / `billed` / `requires_scan` concepts. Machine is now `backorder → pending_approval → approved → ready_to_bill → billed → cancelled`.
- **Billing:** `process_order` now **requires a Tally bill number** — captured + shown on the order and the pick-slip PDF, CHECK-constrained.
- **Universal godown fulfilment:** *all* brands now go through a **godown pick** (LG scans serials; fixed brands enter picked qty). A **partial pick splits off a `Backorder`** child for the un-shipped remainder, which the salesman/admin can **Punch** back into the pipeline. `/scan/[id]` is open to all roles with native `BarcodeDetector` + USB-HID support; a **"PAKKA?"** confirm guards an accidental short pick.
- **Mobile Orders UI revamp:** one unified `OrdersView` / `OrderDetailView` (the offline queue was retired), glyph-driven status tabs + chips, mobile filter header, and strike-through of un-picked units on both the order page **and the pick-slip PDF** (partial-order PDF **owner-verified 2026-07-11**).
- **Cancel/edit permissions (APPLIED to prod 2026-07-11):** the 2-hour edit window is gone; cancel/edit rights are now purely **role × status** matrices (salesman own-pending, accountant pending-only, admin any-later + reason).
- **Perf (Phase 6 #1 done):** `loading.tsx` skeletons + nav-button spinners on every route (instant nav feedback); prod moved to the Mumbai region (Phase 6 #5, same-region item).

Decisions live in [docs/decisions.md](docs/decisions.md); specs in [docs/specs/](docs/specs/) are the source of truth. **Remaining before general rollout: the 1-week parallel-with-notebook pilot** (rollout gate below). Work happens on **feature branches with granular commits**; every commit is reviewed by the REVIEWER in [comments.md](comments.md), and blocking findings are fixed in the very next commit.
...
| **M6** | **Deploy + pilot** — Vercel prod, Supabase prod, real accounts, onboard 1 salesman + accountant | Rollout gate below | 🟨 **Deployed + IN ACTIVE USE (2026-07-11)** — live at [direct-sales-nu.vercel.app](https://direct-sales-nu.vercel.app), real accounts created, E2E-tested, **real orders flowing** (Mumbai region, both tiers); **pending: the formal 1-week parallel-with-notebook pilot** |
...
1. ~~**`loading.tsx` skeletons on every route**~~ — ✅ **Done (2026-07-11, `15cd3f6`)** — skeletons on every route + nav-button spinners (incl. the scan button); navigations give instant feedback instead of freezing until the server render finishes.
...
5. **Cut per-navigation server cost** — the middleware runs `getUser()` (auth-server round-trip) + a `profiles` query on **every** request; carry role/active in the session instead. Add the deferred FK indexes (⑭). ~~Confirm Vercel functions + Supabase are **same region**~~ ✅ **same-region done (2026-07-11)** — Vercel moved to `bom1` (Mumbai), alongside Supabase `ap-south-1`.
