...
>
> **Follow-up `78030c2` (owner repro: "finger on a ribbon scrolls it, one ribbon off-screen; then it stopped"):** a `100dvh` box can transiently exceed the *visible* screen — dvh doesn't track the on-screen keyboard, URL-bar transitions, or leftover page zoom — and then the document scrolls the whole box as a unit. Hardened: phone `.shell` is now `position:fixed; inset:0` (document can never scroll, keyboard or not) + `overscroll-behavior-y:contain` on `.main`. Desktop untouched. Note: the salesman shell (same 100dvh pattern, un-pinned) can in principle show the same transient under a keyboard — owner reports it "gorgeous," so left alone; pin it the same way if ever reported. Owner later confirmed the exact repro (typed in search → reloaded mid-keyboard → bar below the fold) — that's this bug's cleanest trigger; fixed by the pin.
>
> **Follow-up `4090bec` (owner iPhone-vs-Android screenshots):** un-reset `<select>`s render OS-native (iOS WebKit — all iPhone browsers incl. "Chrome" — draws the compact stacked-⌃⌄ control; Android draws a taller down-caret one), and the text `✕` was drawn from each platform's symbol-font fallback at different sizes. Fixed platform-identical: `.pSelect`/`.filterSelect` get `appearance:none` + an inline-SVG chevron; all 3 popups' close buttons swap the text ✕ for the **lucide `X` via Glyph** (SVG — same pixels everywhere, and finally on the app's icon convention). **Builder note:** the salesman page + Quick Order selects are still native-rendered (owner loves those pages; not touched) — if cross-platform parity is ever wanted there, apply the same `appearance:none`+chevron pattern.
>
> **Follow-up `5575207` (owner loves the chevron):** [FilterDropdown](src/components/orders/FilterDropdown.tsx)'s trigger `▾` (text char — same platform-symbol-font problem) → **lucide `ChevronDown` 14px via Glyph**; one shared component, so the Salesman/Brand/Date filters on both the salesman home and the staff dashboard picked it up at once.
>
> **Follow-up `009a709` (owner picks from my discrepancy report):** (1) **status chips pin on phone** — sticky full-bleed paper band at the scroller top in [OrdersView.module.css](src/components/orders/OrdersView.module.css); the `.filters { overflow-x:hidden }` "belt" was REMOVED because an overflow ancestor kills position:sticky (the minmax(0,1fr) grid tracks are the real width containment — watch for any horizontal-scroll regression on exotic content); salesman home + dashboard + godown Home all share OrdersView so all three pin. (2) **QuickOrder + salesman-Products brand selects** get the `appearance:none`+SVG-chevron skin (iOS ⌃⌄ gone app-wide for selects); QuickOrder `:disabled` now tints `background-color` only so the chevron survives. (3) **Owner call recorded: the orders-vs-products filter LOOKS stay distinct** — magnifier-icon/height/style unification explicitly declined ("i like both of them… lets leave them"); do NOT unify later without a fresh owner ask.
>
> **Follow-up `b0f8607` (owner reversed on the magnifier once shown what it was):** the lucide `Search` lens (14px) added to all THREE products search boxes — salesman [ProductsBrowse](src/app/products/ProductsBrowse.tsx), admin phone bar, admin desktop row — via the OrdersView `searchWrap` pattern (wrapper owns border/bg/icon + `:focus-within` accent, naked input inside). Heights/backgrounds/placeholders unchanged; the broader look-unification remains declined (supersedes only the magnifier part of the `009a709` note).
>
...
>
> **Realtime ROOT CAUSE FOUND + FIXED (`05c3bd0` + `eba5311`):** owner reproduced on **Mac Chrome** (not iOS-only) → probed `realtime.subscription` live: **every orders subscription carried `claims_role=anon`** — the channel joined before the user's JWT reached the socket, so walrus evaluated RLS as `anon` (no SELECT on orders) and delivered **zero events silently on every platform, since the feature shipped**. Fix `eba5311`: `getSession()` → `realtime.setAuth(token)` **before** the channel join + an `onAuthStateChange` listener keeping the socket authorized across token refreshes. Belt (`05c3bd0`): render-phase re-seed of `orders` from fresh server payloads (useState-read-once meant router.refresh() changed nothing — latent bug), status-aware `.subscribe()` with 4s rebuild + catch-up refresh, and `visibilitychange`/`online` → `router.refresh()` (socket-free freshness on every app return). Shared OrdersView → salesman home + dashboard + godown all inherit. **BUILDER lesson: any future browser-side `postgres_changes` channel must `setAuth` the session token before `.subscribe()` and handle the status callback — a bare subscribe on an RLS table fails silently as anon.**
